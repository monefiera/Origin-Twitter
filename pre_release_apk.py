...
# 2. Create a new Pre-release on GitHub
def create_github_prerelease(version, release_id):
    headers = {
    ...
    data = {
        ...
        "name": f"Origin Twitter v{version}-release.{release_id} (Pre-release)",
        "body": f"Auto-generated pre-release: Origin Twitter version {version}-release.{release_id}.",
        "draft": False,
        "prerelease": True  # Pre-releaseとして作成
    }
    response = requests.post(GITHUB_API, json=data, headers=headers)
    if response.status_code == 201:
        release_info = response.json()
        print(f"GitHub Pre-release created: {release_info['html_url']}")
        return release_info["id"], release_info["upload_url"].split("{")[0]
    else:
        print(f"Failed to create pre-release: {response.text}")
        return None, None
...
def release_apks():
    ...
    if not version or not release_id:
        return
    github_release_id, upload_url = create_github_prerelease(version, release_id)
